...
# 2020-12-10
#
# This version is same as main scenario, but replaces HC deforestation
# rates with the ones calculated from the Goldberg data 
...
datg <- read_csv("giri-goldberg-rates-of-change.csv")
...
dat <- left_join(dat, datg, by = c("PROVINCE" = "PROVINC"))
dat$rate <- dat$rate * -1
dat$rate[is.na(dat$rate)] <- 0

with(dat, plot(def_rate_instant_HC, rate))
abline(0,1)

#replace defrates 
dat$def_rate_instant_HC <- dat$rate
dat$def_percent_HC <- 100*(exp(dat$def_rate_instant_HC)-1) # Finite rate, for plotting
...
write.csv(luCmax, "2020-12-11_emissions-LU_Goldberg.csv", row.names = FALSE)
write.csv(fract_Cmax, "2020-12-11_emissions-fractions-LU-Goldberg.csv", row.names = FALSE)
...
save(fract_Cmax, file = "Outputs/2020-12-11_fraction-emission-goldberg.rda")
save(emdat, file = "Outputs/2020-12-11_emissions-projections-goldberg.rda")
